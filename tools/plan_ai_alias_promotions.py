...
def plan_ai_alias_promotions(
    ...
    decision_rows = _read_decision_rows(input_csv)
    ...
    for row in decision_rows:
        ...
        if skip_reason:
            ...
            continue
        planned_rows.append(_build_plan_row(row, canonical_sha256_before=canonical_before))
    ...
    summary = PlanSummary(
        ...
        all_batch_id_batch0=all(row["batch_id"] == BATCH0_ID for row in planned_rows),
...
def _read_decision_rows(path: Path) -> list[dict[str, str]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        reader = csv.DictReader(handle)
        fieldnames = set(reader.fieldnames or [])
        missing = set(DECISION_COLUMNS) - fieldnames
        if missing:
            raise ValueError(f"{path} is missing decision columns: {', '.join(sorted(missing))}")
        return [
            {column: (source_row.get(column) or "").strip() for column in DECISION_COLUMNS}
...
def _build_plan_row(row: Mapping[str, str], *, canonical_sha256_before: str) -> dict[str, str]:
    return {
        "raw": row["raw"],
    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        f"- all batch_id={BATCH0_ID}: `{str(summary.all_batch_id_batch0).lower()}`",
